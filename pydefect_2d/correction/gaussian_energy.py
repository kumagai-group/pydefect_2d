...
@dataclass
class GaussianEnergies(MSONable, ToJsonFileMixIn):
...
def make_gaussian_energy(corr_dir: Path, z: float):
    ...
    return GaussianEnergy(z, isolated.self_energy, periodic_energy)


def make_gaussian_energies(corr_dir: Path,
    ...
    result = [make_gaussian_energy(corr_dir, z) for z in zs]
    return GaussianEnergies(result)
